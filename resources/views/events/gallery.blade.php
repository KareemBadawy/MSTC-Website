                            ...
                            @if(Auth::check())
                                @if(Auth::user()->hasRole('Vice Head'))
                                    <div class="interaction cover">
                                        <button  class="btn btn-danger btn-sm delete_image">Deletephoto</button>
                                    </div>
                                @endif
                            @endif
                        </div>
    ...
    $("#myDrop").dropzone({
        ...
        success: function (file, response) {
            ...
            $(img_list).append('<div class="col-xs-6 col-md-3" data-eventid="{{$event->id}}" data-filename="' + response + '\" > ' +
                    ...
                    '              <img class="img-responsive" id="myimage"  src="' + '{{asset('image/Events/' . $event->id) }}' + '/' + response + '\"' + ' alt="..."> ' +
                    '              </a> ' +
                    '              <div class="interaction"> ' +
                    '              <button  class="btn btn-primary btn-sm change_cover" >use as cover</button> ' +
            ...
            $(img_list).append('<div id="' + response + '" class="item"  style="height: 500px;  background-color: black"> <img  src="' + '{{asset('image/Events/' . $event->id) }}' + '/' + response + '"' + '   style="height: 100%; margin-left:auto;margin-right:auto;"   > </div>');
        ...
        removedfile: function (file) {
            if (file.status == 'success') {
                $.ajax({
                    method: 'get',
                    url: '/events/' + '{{$event->id}}' + '/images/' + file.xhr.responseText + '/destroy'
                }).done(function () {
                    var filename = file.xhr.responseText.toString();
                    $('[data-filename~="' + filename + '"]')[0].remove();
                });
            ...
            $.ajax({
                    ...
                    .done(function () {
                        if ($('div.interaction.cover')[0]) {
                            $('div.interaction.cover').append("<button class=\"btn btn-primary btn-sm change_cover\" >use as cover</button>")
